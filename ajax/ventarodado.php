...
require_once "../modelos/VentaRodado.php";
...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idventa)){
			...
			$rspta=$venta->insertar($idrodado,$idusuario,$tipo_comprobante,$fecha_hora,$observacion,$impuesto,$total_venta,$idarticulo,$cantidad,$idubi_origen,$idubi_destino);
	...
	case 'listarArticulos':
        ...
        while ($reg = $rspta->fetch_object()) {
            $data[] = array(
				"0"=> '<a data-toggle="modal" href="#deposiModal"> <button type="button" class="btn btn-warning" onclick="listarArticulosId('.$reg->idarticulo.'); agregarArticuloADeposito('.$reg->idarticulo.','.$reg->idubicacion.',\''.htmlspecialchars($reg->nombre).'\',\''.htmlspecialchars($reg->descripcion).'\',\''.htmlspecialchars($reg->categoria).'\',\''.htmlspecialchars($reg->subcategoria).'\',\''.$reg->codigo.'\',\''.$reg->c_ubi.'\','.$reg->cantidad.');  ">
				<i class="fa fa-eye"></i>
			</button> </a>',
	...
	case 'listarArticulosUno':
		...
		while ($reg = $rspta->fetch_object()) {
			$data[] = array(
				"0"=> '<a data-toggle="modal" href="#deposiModal"> <button type="button" class="btn btn-warning" onclick=" controlIdUno('.$reg->idarticulo.'); agregarArticuloADeposito('.$reg->idarticulo.','.$reg->idubicacion.',\''.htmlspecialchars($reg->nombre).'\',\''.htmlspecialchars($reg->descripcion).'\',\''.htmlspecialchars($reg->categoria).'\',\''.htmlspecialchars($reg->subcategoria).'\',\''.$reg->codigo.'\',\''.$reg->c_ubi.'\','.$reg->cantidad.');">
				<i class="fa fa-eye"></i>
			</button> </a>',
	...
	case 'listarPorFecha':
		$rspta=$venta->ventaPorFecha($idrodado,$fecha_inicio,$fecha_fin);
 		//Vamos a declarar un array
 		$data= Array();
